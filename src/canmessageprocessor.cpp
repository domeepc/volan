...
CanMessageProcessor::CanMessageProcessor(QObject* parent) : QObject{parent} {}

void CanMessageProcessor::setMessages(
    const QList<QCanMessageDescription>& messages) {
  m_processor.setMessageDescriptions(messages);
  qDebug() << "QCanFrameProcessor initialized succesfully with "
           << m_processor.messageDescriptions().count() << " messages.";
}
...
void CanMessageProcessor::processFrame(const QCanBusFrame& frame) {
  ...
  if (result.signalValues.isEmpty()) {
    qDebug() << "Parsing failed: Unique ID " << result.uniqueId
...
void CanMessageProcessor::setUniqueIDDescription(
    const QCanUniqueIdDescription& idDescription) {
